...
@Component({
  ...
  styleUrls: ['./rental-owners.component.css']
})
export class RentalOwnersComponent implements OnInit {
...
displayedColumns: string[] = [
  ...
  // -------------------------
  // LOAD OWNERS (ONLY RENTAL)
  // -------------------------
  loadOwners() {
    const cached = this.ownerService.getCachedOwners(this.agentId);
    if (cached) {
      const rentalOwners = cached.filter(c => c.propertyType === 'Rental'); // filter here
    ...
    this.ownerService.getAllOwners(this.agentId).subscribe({
      next: (data: Owner[]) => {
        const rentalOwners = data.filter(c => c.propertyType === 'Rental'); // filter here
