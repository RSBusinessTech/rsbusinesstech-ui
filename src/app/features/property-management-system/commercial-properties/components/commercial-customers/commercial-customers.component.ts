...
@Component({
  ...
  styleUrls: ['./commercial-customers.component.css']
})
export class CommercialCustomersComponent implements OnInit {
...
displayedColumns: string[] = [
  ...
  // -------------------------
  // LOAD CUSTOMERS (ONLY Commercial)
  // -------------------------
  loadCustomers() {
    const cached = this.customerService.getCachedCustomers(this.agentId);
    if (cached) {
      const rentalCustomers = cached.filter(c => c.propertyType === 'Commercial'); // filter here
    ...
    this.customerService.getAllCustomers(this.agentId).subscribe({
      next: (data: Customer[]) => {
        const rentalCustomers = data.filter(c => c.propertyType === 'Commercial'); // filter here
