...
@Component({
  selector: 'app-new-projects-owners',
  templateUrl: './new-projects-owners.component.html',
  styleUrls: ['./new-projects-owners.component.css']
})
export class NewProjectsOwnersComponent implements OnInit {
 ...
 displayedColumns: string[] = [
  ...
  // -------------------------
  // LOAD OWNERS (ONLY New Project)
  // -------------------------
  loadOwners() {
    const cached = this.ownerService.getCachedOwners(this.agentId);
    if (cached) {
      const rentalOwners = cached.filter(c => c.propertyType === 'New Project'); // filter here
    ...
    this.ownerService.getAllOwners(this.agentId).subscribe({
      next: (data: Owner[]) => {
        const rentalOwners = data.filter(c => c.propertyType === 'New Project'); // filter here
