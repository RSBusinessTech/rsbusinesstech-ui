...
@Component({
  selector: 'app-new-projects-customers',
  templateUrl: './new-projects-customers.component.html',
  styleUrls: ['./new-projects-customers.component.css']
})
export class NewProjectsCustomersComponent implements OnInit {
  ...
  displayedColumns: string[] = [
      ...
      'rentalStartDate','rentalDueDate','contractStartDate','contractEndDate','isRentalPaid','propertyPrice','stampDutyFee','registrationFee','downPaymentAmount',
      'monthlyInstallmentAmount','numberOfInstallments','purchaseDate','imageUrl','createdBy','createdAt','updatedBy','updatedAt','actions'
    ];
    ...
    ngOnInit() {
      ...
      // Filter predicate (only for search, Commercial type enforced in loadCustomers)
    ...
    // -------------------------
    // LOAD CUSTOMERS (ONLY New Project)
    // -------------------------
    loadCustomers() {
      const cached = this.customerService.getCachedCustomers(this.agentId);
      if (cached) {
        const rentalCustomers = cached.filter(c => c.propertyType === 'New Project'); // filter here
      ...
      this.customerService.getAllCustomers(this.agentId).subscribe({
        next: (data: Customer[]) => {
          const rentalCustomers = data.filter(c => c.propertyType === 'New Project'); // filter here
    ...
    addNewRow() {
      const newRow: Customer = {
        ...
        propertyType: 'New Project', // default to New Project
        fullName: '',
        fatherName: '',
        dateOfBirth: '',
        customerID: '',
