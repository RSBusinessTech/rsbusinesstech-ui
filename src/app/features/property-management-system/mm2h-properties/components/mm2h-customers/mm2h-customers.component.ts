...
@Component({
  selector: 'app-mm2h-customers',
  templateUrl: './mm2h-customers.component.html',
  styleUrls: ['./mm2h-customers.component.css']
})
export class Mm2hCustomersComponent implements OnInit {
...
displayedColumns: string[] = [
      ...
      'whatsappNumber','addressLine1','addressLine2','city','state','postalCode','country','accountStatus','preferredContactMethod','gender','propertyPrice',
      'stampDutyFee','registrationFee','downPaymentAmount','monthlyInstallmentAmount','numberOfInstallments','purchaseDate','imageUrl','createdBy','createdAt',
  ...
  loadCustomers() {
    ...
    if (cached) {
      const rentalCustomers = cached.filter(c => c.propertyType === 'MM2H'); // filter here
    ...
    this.customerService.getAllCustomers(this.agentId).subscribe({
      next: (data: Customer[]) => {
        const rentalCustomers = data.filter(c => c.propertyType === 'MM2H'); // filter here
  ...
  addNewRow() {
    const newRow: Customer = {
      ...
      propertyType: 'MM2H', // default to MM2H
      fullName: '',
      fatherName: '',
      dateOfBirth: '',
      customerID: '',
