...
@Component({
  selector: 'app-mm2h-owners',
  templateUrl: './mm2h-owners.component.html',
  styleUrls: ['./mm2h-owners.component.css']
})
export class Mm2hOwnersComponent implements OnInit {
  ...
  // -------------------------
  // LOAD OWNERS (ONLY MM2H)
  // -------------------------
  loadOwners() {
    const cached = this.ownerService.getCachedOwners(this.agentId);
    if (cached) {
      const rentalOwners = cached.filter(c => c.propertyType === 'MM2H'); // filter here
    ...
    this.ownerService.getAllOwners(this.agentId).subscribe({
      next: (data: Owner[]) => {
        const rentalOwners = data.filter(c => c.propertyType === 'MM2H'); // filter here
