...
@Component({
  selector: 'app-buy-owners',
  templateUrl: './buy-owners.component.html',
  styleUrls: ['./buy-owners.component.css']
})
export class BuyOwnersComponent implements OnInit {
  ...
  // -------------------------
  // LOAD OWNERS (ONLY Buy)
  // -------------------------
  loadOwners() {
    const cached = this.ownerService.getCachedOwners(this.agentId);
    if (cached) {
      const rentalOwners = cached.filter(c => c.propertyType === 'Buy'); // filter here
    ...
    this.ownerService.getAllOwners(this.agentId).subscribe({
      next: (data: Owner[]) => {
        const rentalOwners = data.filter(c => c.propertyType === 'Buy'); // filter here
