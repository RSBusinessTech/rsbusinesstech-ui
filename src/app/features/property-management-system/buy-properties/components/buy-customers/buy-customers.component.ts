...
@Component({
  ...
  templateUrl: './buy-customers.component.html',
  styleUrls: ['./buy-customers.component.css']
})
export class BuyCustomersComponent implements OnInit {
  ...
  loadCustomers() {
    ...
    if (cached) {
      const rentalCustomers = cached.filter(c => c.propertyType === 'Buy'); // filter here
    ...
    this.customerService.getAllCustomers(this.agentId).subscribe({
      next: (data: Customer[]) => {
        const rentalCustomers = data.filter(c => c.propertyType === 'Buy'); // filter here
  ...
  addNewRow() {
    const newRow: Customer = {
      ...
      propertyType: 'Buy', // default to Buy
      fullName: '',
      fatherName: '',
      dateOfBirth: '',
      customerID: '',
